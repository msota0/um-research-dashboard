...
export default function Authors({ onOaError }: Props) {
  ...
  const openModal = async (author: Author) => {
    ...
    const [worksRes, expertiseRes] = await Promise.allSettled([
      ...
      api.authorExpertise(author.id, author.orcid ?? undefined),
    ]);

    if (worksRes.status === 'fulfilled') {
      setModalWorks(worksRes.value.data);
    }

    if (expertiseRes.status === 'fulfilled') {
      setExpertiseKeywords(expertiseRes.value.data);
    }
          ...
          <div className={styles.modal} onClick={e => e.stopPropagation()}>
            ...
            <div className={styles.modalStats}>
              <div className={styles.modalStat}>
                <span className={styles.modalStatVal}>
                  {fmt(modalAuthor.total_publications ?? 0)}
                </span>
                <span className={styles.modalStatLabel}>Total Publications</span>
              </div>

              <div className={styles.modalStat}>
                <span className={styles.modalStatVal}>
                  {fmt(modalAuthor.um_publications ?? 0)}
                </span>
                <span className={styles.modalStatLabel}>UM Publications</span>
              </div>

              <div className={styles.modalStat}>
                <span className={styles.modalStatVal}>
                  {fmt(modalAuthor.cited_by_count)}
                </span>
