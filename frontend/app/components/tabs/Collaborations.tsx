...
/** ISO alpha-2 code → full English country name (e.g. "US" → "United States").
 *  Uses the built-in Intl API so no lookup table needs maintaining. */
const regionNames = typeof Intl !== 'undefined' && 'DisplayNames' in Intl
  ? new Intl.DisplayNames(['en'], { type: 'region' })
  : null;
const countryName = (code: string): string => {
  const cc = isoCode(code);
...
/* Resolve each institution to map coords: city-level from INST_COORDS when we
 * know it, otherwise its country's centroid. Multiple institutions in the same
 * country are fanned out around the centroid (golden-angle spiral) so they don't
 * stack — this is why international institutions no longer pile up over the US. */
function buildInstCoords(insts: CollabInstitution[]): Record<string, [number, number]> {
  const out: Record<string, [number, number]> = {};
  const perCountry: Record<string, number> = {};
  // Largest collaborators first, so the biggest bubble sits on the centroid.
  const ordered = [...insts].sort((a, b) => b.count - a.count);
  for (const it of ordered) {
    if (INST_COORDS[it.name]) { out[it.name] = INST_COORDS[it.name]; continue; }
    const base = COUNTRY_COORDS[isoCode(it.country ?? '')];
    if (!base) continue;  // unknown country → not plottable
    const k = perCountry[it.country] = (perCountry[it.country] ?? 0) + 1;
    if (k === 1) {
      out[it.name] = base;
    } else {
      const angle  = (k - 1) * 2.39996;          // golden angle (radians)
      const radius = 2 + 1.1 * (k - 2);           // grow the spiral outward
      out[it.name] = [base[0] + radius * Math.sin(angle), base[1] + radius * Math.cos(angle)];
    }
  }
  return out;
}
...
interface BubbleMapProps {
  items: BubbleItem[];
  /** Resolve an item to [lat, lon]; return undefined to skip plotting it. */
  resolve: (item: BubbleItem) => [number, number] | undefined;
  bubbleColors?: string[];
  /** Noun used in the accessible map summary, e.g. "countries" or "institutions". */
  summaryNoun?: string;
}

function BubbleMap({ items, resolve, bubbleColors, summaryNoun = 'collaborators' }: BubbleMapProps) {
  ...
  const mapSummary = items.length
    ? `Map of collaborating ${summaryNoun}. Top: ${[...items].sort((a, b) => b.count - a.count).slice(0, 5).map(i => `${i.name} (${i.count})`).join(', ')}.`
    : 'Collaboration map with no data.';

  return (
    <div ref={wrapRef} className={styles.mapWrapper}>
      {!ready && <Skeleton height={290} />}
      <svg ref={svgRef} width="100%" role="img" aria-label={mapSummary} style={{ display: ready ? 'block' : 'none' }} />
      {tooltip && (
...
export default function Collaborations({ onOaError }: Props) {
  ...
  // Institutional view = domestic only (US-based institutions).
  const usInstitutions = (institutions ?? []).filter(d => isoCode(d.country ?? '') === 'US');
  const instLabels    = usInstitutions.map(d => truncate(d.name));
  const instData      = usInstitutions.map(d => d.count);
  const instItems     = usInstitutions.map(d => ({ name: d.name, count: d.count }));

  // International view = everything except the US.
  const intlCountries = (countries ?? []).filter(d => isoCode(d.country_code) !== 'US');
  const countryItems  = intlCountries.map(d => ({ name: countryName(d.country_code), code: isoCode(d.country_code), count: d.count }));
  const countryTop15  = countryItems.slice(0, 15);

  // Per-institution coords, recomputed only when the institution list changes.
  const instCoordsByName = useMemo(() => buildInstCoords(usInstitutions), [institutions]); // eslint-disable-line react-hooks/exhaustive-deps

  return (
    <div className={`${styles.root} fadeInUp`}>

      {/* Two collaboration maps: institutions (placed by country centroid, fanned
          out so same-country institutions don't stack) and countries. */}
      <div className={styles.maps}>
        <div className={styles.mapCard}>
          <div className={styles.mapHeader}>
            <h3 className={styles.mapTitle}>Institutional Collaboration Map</h3>
          </div>
          {institutions
            ? <BubbleMap
                items={instItems}
                resolve={it => instCoordsByName[it.name]}
                bubbleColors={CHART_COLORS}
                summaryNoun="institutions"
              />
            : <Skeleton height={290} />
          }
        </div>
        ...
        <div className={styles.mapCard}>
          <div className={styles.mapHeader}>
            <h3 className={styles.mapTitle}>International Collaboration Map</h3>
          </div>
          {countries
            ? <BubbleMap
                items={countryItems}
                resolve={it => COUNTRY_COORDS[it.code ?? '']}
                bubbleColors={CHART_COLORS}
      ...
      {/* Two divided graphs: institutional (left) + international (right) */}
      <div className={styles.charts}>
        <ChartCard title="Institutional Collaborations" source="openalex" tall>
          {institutions
            ? <div style={{ height: 500 }}>
        ...
        <ChartCard title="International Collaborations" source="openalex" tall>
          {countries
            ? <div style={{ height: 500 }}>
                <HorizontalBarChart
                  labels={countryTop15.map(d => d.name)}
                  data={countryTop15.map(d => d.count)}
                  colors={countryTop15.map((_, i) => CHART_COLORS[i % CHART_COLORS.length])}
                  xFormatter={fmt}
                />
      ...
      {/* Country table — international only (US excluded). */}
      {intlCountries.length > 0 && (
        <div className={styles.countryCard}>
          <div className={styles.countryHeader}>
            <h3 className={styles.countryTitle}>All International Collaborating Countries</h3>
          </div>
          <div className={styles.countryGrid}>
            {intlCountries.map((c, i) => (
              <div key={isoCode(c.country_code)} className={styles.countryRow}>
                <span className={styles.countryRank}>{i + 1}</span>
                <span className={styles.countryName}>{countryName(c.country_code)}</span>
